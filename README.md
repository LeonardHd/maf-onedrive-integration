...
* **AI file summarization** — select any file and get a concise summary
  generated by an LLM (GitHub Models) using the
...
| AI summarization | Microsoft Agent Framework (`agent-framework`) |
...
A setup script is provided that creates the app registration, configures
permissions, and generates a client secret using the Azure CLI:
...
./scripts/setup-app-registration.sh            # uses default name "MAF OneDrive Browser"
./scripts/setup-app-registration.sh "My App"   # or provide a custom display name
```

The script will:

1. Create an app registration with a **Web** redirect URI (`http://localhost:8000/auth/callback`).
2. Configure the required **Delegated** Microsoft Graph permissions (`User.Read`, `Files.Read.All`, `Sites.Read.All`).
3. Generate a client secret (valid for 1 year).
4. Print the `APPLICATION_ID`, `APPLICATION_SECRET`, and `TENANT_ID` values ready to paste into your `.env` file.

> **Requirements:** [Azure CLI](https://learn.microsoft.com/en-us/cli/azure/install-azure-cli) (`az`) and [`jq`](https://jqlang.github.io/jq/) must be installed.

### Manual setup

If you prefer to set up the registration manually:
...
interactively downloads files from a SharePoint document library to your local
machine.

### Prerequisites

Sign in with the Azure CLI so `DefaultAzureCredential` can acquire a token:
...
|---|---|---|
| **SharePoint hostname** | `contoso` or `contoso.sharepoint.com` | `.sharepoint.com` is appended automatically if omitted |
| **Site path** | `/sites/my-team` or `sites/my-team` | Leading `/` is added automatically if omitted |
| **Folder path** | `General` *(or leave empty)* | Optional — press Enter to list the document library root |

See the screenshot below for reference:
![docs/sharepoint_url.png](docs/sharepoint_url.png)
...
SharePoint hostname (e.g. contoso.sharepoint.com): contoso
Site path (e.g. /sites/my-team): sites/Leadership
Folder path (leave empty for root): General

INFO | Resolving drive for contoso.sharepoint.com/sites/Leadership …
INFO | Drive ID: b!UehZ…
INFO | Listing files in /General …
INFO | Found 3 file(s):
INFO |   • report.docx  (24531 bytes)
INFO |   • budget.xlsx   (18209 bytes)
INFO |   • notes.pdf     (102400 bytes)
INFO |   ✓ Saved downloads/report.docx
INFO |   ✓ Saved downloads/budget.xlsx
INFO |   ✓ Saved downloads/notes.pdf
INFO | Done - 3 file(s) downloaded to downloads
```
